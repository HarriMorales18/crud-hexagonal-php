...
(function (): void {
    $requestPath = rtrim(
        str_replace('\\', '/', (string) parse_url((string) ($_SERVER['REQUEST_URI'] ?? '/'), PHP_URL_PATH)),
        '/'
    );
    // Base pública en formato URL (siempre con '/'): /foo/public/index.php -> /foo/public
    $scriptName = str_replace('\\', '/', (string) ($_SERVER['SCRIPT_NAME'] ?? '/index.php'));
    $publicBase = preg_replace('#/index\.php$#i', '', $scriptName) ?? '';
    ...
    if ($requestPath !== $publicBase && !str_starts_with($requestPath, $publicBase . '/')) {
        ...
        $redirectPath = ($publicBase === '')
            ? '/index.php?route=' . urlencode($dest)
            : $publicBase . 'index.php?route=' . urlencode($dest);
        $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
        $host = (string) ($_SERVER['HTTP_HOST'] ?? 'localhost');
        header('Location: ' . $scheme . '://' . $host . $redirectPath);
        exit;
    }
})();
// ──────────────────────────────────────────────────────────────────────────────
